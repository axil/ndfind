...
def test4():
    a = [1,3,4,7,9]
    assert find(a, 1., sorted=True) == 0
    assert find(a, 4., sorted=True) == 2
    assert find(a, 9., sorted=True) == 4
    assert find(a, 5., sorted=True) == -1
    assert find(a, 10., sorted=True) == -1
    assert find(a, 0., sorted=True) == -1
...
def test_datatypes():
    ...
    a = np.arange(np.datetime64('2023-01-20'),
                  np.datetime64('2023-01-23')); a
...
def test_special_floats():
    a = np.array([0., 2., np.nan, np.inf, np.inf, np.NINF, np.nan, np.NZERO, np.PZERO]); a
...
def test_special_complex():
    a = np.array([0., 2j, np.nan, np.inf, np.inf, np.NINF, np.nan, np.NZERO, np.PZERO]); a
    assert find(a, np.NZERO) == 0
    assert find(a, np.PZERO) == 0
    assert find(a, 0.) == 0
    assert find(a, np.nan) == 2
